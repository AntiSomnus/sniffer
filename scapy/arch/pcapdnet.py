...
global pkt_lst,num_capture,num_process,packet
from datetime import datetime
pkt_lst = []
num_capture=num_process=0
if conf.use_dnet:
    try:
        from .cdnet import *
    except OSError as e:
...
if conf.use_winpcapy:
    ...
    class _PcapWrapper_pypcap:
        def __init__(self, device, snaplen, promisc, to_ms):
            ...
            pcap_setbuff(self.pcap, 9999999)
    ...
    class L2pcapListenSocket(SuperSocket):
        ...
        def __init__(self, iface=None, type=ETH_P_ALL, promisc=None, filter=None):
            ...
            self.ins = open_pcap(iface, 65545, self.promisc, 100000)  # change to enlarge ring buffer size
            try:
        ...
        def recv_pkt_lst_init(self):
            t = Thread(target=self.recv_pkt_lst)
            t.start()
    ...
    class L2pcapSocket(SuperSocket):
        ...
        def __init__(self, iface=None, type=ETH_P_ALL, filter=None, nofilter=0):
            if iface is None:
                iface = conf.iface
            self.iface = iface
            self.ins = open_pcap(iface, 65535, 0, 1000)
            try:
                ioctl(self.ins.fileno(), BIOCIMMEDIATE, struct.pack("I", 1))
            except:
                pass
